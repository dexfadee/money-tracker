...
const handleDelete = async () => {
    const res = await axios.delete('/api/delete-transaction')
    ...
    return (
        <ScrollArea className="h-[calc(88vh-152px)] w-full rounded-md py-2 px-4 border border-gray-800">
            ...
            <Table className="px-4">
                ...
                <TableBody>
                    {
                        transactions.map((transaction: { _id: string, text: string, amount: number, isPending: boolean, user: string }) => (
                            <TableRow key={transaction._id} className={`bg-opacity-15` + (transaction.amount > 0 ? ` bg-green-600` : ` bg-red-600`)}>
                                ...
                                <TableCell className="text-right">
                                    <AlertDialog>
                                        <AlertDialogTrigger disabled={!transaction.isPending} className="border border-gray-700 p-2 rounded-md px-4">
                                            {transaction.isPending ? "Completed" : "Pending"}
                                        </AlertDialogTrigger>
                                        <AlertDialogContent>
                                            <AlertDialogHeader>
